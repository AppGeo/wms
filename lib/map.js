...
var abaculus = Promise.promisify(_abaculus);
...
var getMap = Promise.coroutine(function * getMap(layer, rawParams) {
  ...
  try {
    _abaculus.coordsFromBbox(zoom, scale, bbox, MAX_SIZE);
  } catch(e) {
    return makeError(e.message, 'InvalidDimensionValue');
  }
  var image, resizedImage;
  try {
    image = yield abaculus(opts);
  } catch (e) {
    return makeUnknownError(e);
  }
  if (abort.aborted) {
  ...
  try {
    resizedImage = yield resize(image, format, width, height);
  } catch(e) {
...
module.exports = function (layer, params, cache, callback) {
  ...
  return getMap(layer, params, cache).asCallback(callback);
};
...
function noop() {}
var parallel = Promise.coroutine(function * (list, fun) {
  var i = list.length;
  var out = [];
  while (i--) {
    let result = yield fun(list[i]);
    if (!result) {
      continue;
    }
    out.push(result);
