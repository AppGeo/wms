...
function filterAbstract (abstract) {
  ...
  if (abstract.indexOf(']]>') > -1) {
    return 'Abstract Contains invalid sequences.';
  }
  return abstract;
}
...
async function  getCapabilities(layers, rawParams) {
  ...
  var out = {
    title: layers.title || '',
    abstract: filterAbstract(layers.abstract),
  ...
  var data = Buffer.from(template(out));
  var headers = {
...
function makeLayers(layers, tileSets) {
  ...
  return layers.map(function (layer) {
    ...
    var out = {
      ...
      abstract: filterAbstract(layer.abstract),
      png: true,
      jpeg: true
    };
